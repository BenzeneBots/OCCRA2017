...
#define SP_MULT		0.50	// Speed Step-Down Multiplier (for training)
...
#define ARM_FWD_SP	70		// Robot forward speed while arm is deployed.
...
task main () {
	...
	while( true ) {
		...
		if( tmLED < time1[timer1] ) {
			tmLED = time1[timer1] + 25;

			if( ledDir ) {
				if( ledVal <= 127 &&  ledDir ) ledVal += 1;
				if( ledVal >= 0   && !ledDir ) ledVal -= 1;

				if( ledVal == 127 &&  ledDir ) ledDir = 0;
				if( ledVal == 0   && !ledDir ) ledDir = 1;
			}
			motor[ledStrips] = ledVal;
		}
